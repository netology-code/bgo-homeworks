...
Но здесь есть тонкий момент: а что если в каком-то месяце не было транзакций? При таком подходе у вас в отчёте вообще этого месяца не будет, хотя должен был быть со значением 0.
