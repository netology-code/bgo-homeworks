...
Примечание*: обратите внимание, сколько бы мы ни писали "в рублях", всё равно хранить внутри надо в копейках.

<details>
  <summary>Рубли vs Копейки 😈</summary>
  
  Вы можете посмотреть реализацию в популярных банках:
  
  1\. [Сбербанк](https://developer.sberbank.ru/doc/v1/acquiring/rest-requests1pay):
  
  ![](pic/sberbank.png)
  
  2\. [Альфабанк](https://pay.alfabank.ru/ecommerce/instructions/merchantManual/pages/index/rest.html):
  
  ![](pic/alfabank.png)
  
  3\. [Тинькофф](https://oplata.tinkoff.ru/develop/api/payments/init-request/)
  
  ![](pic/tinkoff.png)
  
  Но в то же время, вы должны понимать, что нет ничего "абсолютного", и вполне себе существуют API, которые требуют значения сумм в рублях или долларах (а как оно хранится внутри в БД уже дело самих разработчиков).
</details>
...
Для того, чтобы ваши тесты автоматически запускались с помощью GitHub Actions (при push в GitHub), создайте в вашем проекте файл `.github/workflows/go.yml` (обратите внимание на точку в имени первого каталога):
```yml
name: Go
...
Примечание*: вам ничего менять не нужно, если хотите детальнее ознакомиться с системой GitHub Actions, то [обратитесь к документации](https://github.com/features/actions).

<details>
<summary>Картинки про GitHub Actions</summary>

Если вы скопируете конфигурацию неправильно, GitHub Actions сообщит вам об этом (мы удалили у первого слова первую букву):

![](pic/actions1.png)

После того, как вы запушите корректный файл конфигурации, на вкладке GitHub Actions можно посмотреть прогресс выполнения:
...
Для каждого пуша, для которого производилась сборка, на всех страницах GitHub будет отображаться статус (иконки статуса кликабельны):
...
Убедитесь, что тесты запускаются и проходят, с помощью команды `go test -v ./...`, выполненной в корневом каталоге проекта и при пушах в GitHub Actions.
